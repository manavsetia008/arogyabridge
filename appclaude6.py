...
# # Streamlit Prototype (MVP v4)
# #
# # v4 adds seven features not commonly found in similar rural-telehealth
# # submissions, layered on top of the existing v3 feature set. See README.md
# # for the full "what was already there vs. what's new" breakdown.
# #
# # Run:
...
# #
# # Optional:
# #     export GROQ_API_KEY=...      # enables real LLM triage + voice-to-text
...
# # --- NEW (v4): accountability SLAs ------------------------------------------
# # The problem statement explicitly asks for solutions that improve
# # "accountability" in the referral chain. These are the maximum hours a
# # referral should sit un-acknowledged by the receiving facility before it
# # counts as an SLA breach, shown to both the facility and district admin.
# SLA_HOURS = {
#     "RED": 2,       # emergency referrals must be acknowledged within 2 hours
#     "YELLOW": 12,   # routine-urgent referrals within 12 hours
#     "GREEN": None,  # GREEN cases are not referred
# }

# # --- NEW (v4): offline, zero-cost text-to-speech language codes -------------
# TTS_LANG_CODES = {
#     "en": "en-IN", "hi": "hi-IN", "mr": "mr-IN",
#     "ta": "ta-IN", "te": "te-IN", "bn": "bn-IN",
# }

# # Canned, pre-translated vernacular phrases read aloud via the browser's
# # built-in Web Speech API — no internet call, no API key, works even if the
# # ASHA worker's phone has zero connectivity at that exact moment.
# PRIORITY_SPEECH = {
#     "en": {"RED": "This is an emergency. Please go to the hospital immediately.",
#            "YELLOW": "Please visit the health centre soon for a check-up.",
#            "GREEN": "This is routine. Home care is enough for now."},
#     "hi": {"RED": "यह एक आपातकालीन स्थिति है। कृपया तुरंत अस्पताल जाएं।",
#            "YELLOW": "कृपया जल्द ही जांच के लिए स्वास्थ्य केंद्र जाएं।",
#            "GREEN": "यह सामान्य स्थिति है। अभी घर पर देखभाल पर्याप्त है।"},
#     "mr": {"RED": "ही आणीबाणीची स्थिती आहे. कृपया ताबडतोब रुग्णालयात जा.",
#            "YELLOW": "कृपया लवकरच तपासणीसाठी आरोग्य केंद्रात जा.",
#            "GREEN": "ही सर्वसाधारण स्थिती आहे. सध्या घरगुती काळजी पुरेशी आहे."},
#     "ta": {"RED": "இது ஒரு அவசர நிலை. உடனடியாக மருத்துவமனைக்குச் செல்லவும்.",
#            "YELLOW": "விரைவில் பரிசோதனைக்காக சுகாதார மையத்திற்குச் செல்லவும்.",
#            "GREEN": "இது வழக்கமான நிலை. தற்போது வீட்டு பராமரிப்பு போதுமானது."},
#     "te": {"RED": "ఇది అత్యవసర పరిస్థితి. వెంటనే ఆసుపత్రికి వెళ్లండి.",
#            "YELLOW": "త్వరలో పరీక్ష కోసం ఆరోగ్య కేంద్రానికి వెళ్లండి.",
#            "GREEN": "ఇది సాధారణ పరిస్థితి. ప్రస్తుతం ఇంటి సంరక్షణ సరిపోతుంది."},
#     "bn": {"RED": "এটি একটি জরুরি অবস্থা। অনুগ্রহ করে অবিলম্বে হাসপাতালে যান।",
#            "YELLOW": "অনুগ্রহ করে শীঘ্রই পরীক্ষার জন্য স্বাস্থ্য কেন্দ্রে যান।",
#            "GREEN": "এটি একটি রুটিন অবস্থা। এখন বাড়িতে যত্ন যথেষ্ট।"},
# }
...
#         "facility_load_hint": "Current pending referral load across facilities",
#         "predicted_stockout": "Predicted stockout",
#         "audit_trail": "Consent & Access Audit Trail",
#         "voice_input_label": "🎙️ Record symptoms (optional, voice-to-text)",
#         "voice_transcribing": "Transcribing voice note...",
#         "facility_health_score": "Facility Health Score",
#         "listen_label": "🔊 Listen / सुनें",
#     },
#     "hi": {
...
#     # -------------------------------------------------------------------------
#     # Seed medicine inventory (+ NEW v4: baseline consumption history so the
#     # predictive stockout forecaster has data to work with from minute one)
#     # -------------------------------------------------------------------------
...
#                     VALUES (?, ?, ?, ?, ?, ?)
#                     """,
#                     (stock_id, facility, medicine, quantity, unit, now),
#                 )

#                 cur.execute(
#                     """
#                     INSERT INTO medicine_stock_history
#                     (id, stock_id, facility, medicine_name, quantity_after, change_amount, changed_at)
#                     VALUES (?, ?, ?, ?, ?, ?, ?)
#                     """,
#                     (str(uuid.uuid4()), stock_id, facility, medicine, quantity, quantity, now),
#                 )
...
# # --- NEW (v4): offline, zero-API-cost text-to-speech --------------------------
# def speak_button(text: str, lang_code: str = "en-IN", key: str = ""):
#     """Renders a button that reads `text` aloud using the browser's built-in
#     Web Speech API (speechSynthesis). This runs entirely client-side — no
#     network call, no API key, no server round-trip — so it keeps working
#     even if the phone loses signal right after the page has loaded. This
#     directly extends the "vernacular support" USP from voice *input* (which
#     needs the AI service) to voice *output* (which does not)."""
#     safe_text = (
#         text.replace("\\", "")
#         .replace("`", "'")
#         .replace("\n", " ")
...
#         " style="background:#12806B;color:white;border:none;padding:8px 16px;
...
# # --- NEW (v4): accountability helpers -----------------------------------------
# def hours_since(iso_timestamp):
#     """Returns hours elapsed since an ISO timestamp, or None if unparsable."""
...
# def facility_pending_load():
#     """Returns {facility: pending_referral_count} — used to warn an ASHA
#     worker at the point of referral if the facility they're about to choose
#     is already overloaded with un-acknowledged referrals."""
#     result = df(
#         "SELECT facility, COUNT(*) AS n FROM referrals WHERE status='PENDING' GROUP BY facility"
#     )
#     loads = {f: 0 for f in FACILITIES}
#     for _, row in result.iterrows():
#         if row["facility"] in loads:
#             loads[row["facility"]] = int(row["n"])
#     return loads


# # --- NEW (v4): predictive medicine stockout forecasting ------------------------
# def predict_stockout(stock_id):
#     """Returns estimated days until stockout based on actual consumption
#     velocity recorded in medicine_stock_history, or None if there isn't
#     enough history to forecast confidently yet. This goes beyond a static
#     "below 10 units" threshold by using real depletion trend data."""
#     history = df(
#         "SELECT quantity_after, change_amount, changed_at FROM medicine_stock_history "
#         "WHERE stock_id = ? ORDER BY changed_at",
#         (stock_id,),
#     )
#     if len(history) < 2:
#         return None

#     history["changed_at"] = pd.to_datetime(history["changed_at"], errors="coerce")
#     consumption = history[history["change_amount"] < 0]
#     if consumption.empty:
#         return None  # no depletion observed yet — stable

#     total_consumed = -consumption["change_amount"].sum()
#     span_days = (history["changed_at"].max() - history["changed_at"].min()).total_seconds() / 86400
#     span_days = max(span_days, 1)
#     avg_daily_consumption = total_consumed / span_days
#     if avg_daily_consumption <= 0:
#         return None

#     current_quantity = history.iloc[-1]["quantity_after"]
#     days_remaining = current_quantity / avg_daily_consumption
#     return round(float(days_remaining), 1)
...
# # =============================================================================
# # NEW (v4): CONSENT & ACCESS AUDIT TRAIL
# # =============================================================================
# # ABDM's consent-manager model is about more than storing data — it's about
# # being able to show WHO looked at or acted on a patient's record and WHY.
# # This lightweight audit log gives the district admin exactly that view
# # without needing a full ABDM Consent Manager integration.

# def log_access(abha_id: str, accessed_by: str, purpose: str):
#     conn = get_conn()
#     conn.execute(
#         "INSERT INTO access_log (id, abha_id, accessed_by, purpose, created_at) VALUES (?,?,?,?,?)",
#         (str(uuid.uuid4()), abha_id, accessed_by, purpose, datetime.now().isoformat()),
#     )
#     conn.commit()
#     conn.close()
...
# # --- NEW (v4): voice-to-text symptom capture (Groq Whisper) --------------------
# def transcribe_voice_note(audio_bytes: bytes):
#     """Sends a recorded voice note to Groq's Whisper endpoint for
#     transcription. Returns the transcript text, or None if unavailable
#     (no API key, no network, or any SDK error) — the caller always falls
#     back gracefully to manual typing."""
...
#             file=("voice_note.wav", audio_bytes),
...
#         "been", "very", "mild", "moderate", "severe", "days", "day", "since",
#         "after", "before", "also", "today", "there", "feeling", "feels",
#         "gets", "getting",
#     }
...
#     people = [
#         ("Sunita Pawar", 34, "Female", "Wagholi", "9876501234", "Marathi"),
#         ("Rahul Jadhav", 45, "Male", "Ranjangaon", "9876505678", "Hindi"),
#         ("Geeta Patil", 27, "Female", "Wagholi", "9876509012", "Marathi"),
#         ("Imran Sheik", 60, "Male", "Shirval", "9876503456", "Urdu"),
#         ("Meena Mandale", 52, "Female", "Lalgaon", "9876507890", "Marathi"),
#         ("Vikram Solu", 8, "Male", "Shirval", "9876502345", "Hindi"),
#         ("Aarti Kuwar", 22, "Female", "Chaki", "9876508901", "Hindi"),
#     ]
...
#             (
#                 abha_id, name, age, gender, village, phone, language,
#                 ago(hours=30 - i * 3), None, None,
#                 asha_names[i % len(asha_names)],
#             ),
#         )

#     triage_rows = [
#         (0, "High fever for 3 days with cough and body ache", "YELLOW",
#          "Fever detected. Needs facility follow-up.", "PHC Ranjangaon",
#          "ACKNOWLEDGED", 3, 2, None, "Dr. Neha (PHC)"),
#         (1, "Breathlessness and chest pain since morning", "RED",
#          "Breathlessness and chest pain require immediate escalation.",
#          "District Hospital Pune", "COMPLETED", 26, 24, 22, "Dr. Anil (DH)"),
#         (2, "Vomiting and diarrhea, feels dehydrated", "YELLOW",
#          "Vomiting and diarrhea with dehydration risk.", "PHC Ranjangaon",
#          "COMPLETED", 20, 19, 18, "Dr. Neha (PHC)"),
#         (4, "Frequent urination, fatigue and dizziness", "YELLOW",
#          "Needs Hb and diabetes evaluation.", "Sub-Centre Wagholi",
#          "COMPLETED", 15, 14, 13, "Dr. Kavita (SCW)"),
#         (5, "Mild fever and headache, no other complaints", "GREEN",
#          "No high-risk symptoms detected.", None, None, 12, None, None, None),
#         (3, "Unconscious for a few minutes after a seizure", "RED",
#          "Unconsciousness and seizure require emergency escalation.",
#          "District Hospital Pune", "PENDING", 1, None, None, None),
#         (6, "Skin rashes and itching after changing feed", "GREEN",
#          "No high-risk symptoms detected.", None, None, 6, None, None, None),
#     ]

#     for (index, symptoms, priority, rationale, facility, status,
#          created_hours, ack_hours, completed_hours, doctor) in triage_rows:
...
#             """
#             INSERT INTO triage
#             (id, abha_id, symptoms_text, priority, rationale, created_by, created_at)
#             VALUES (?, ?, ?, ?, ?, ?, ?)
#             """,
#             (triage_id, abha_map[index], symptoms, priority, rationale,
#              asha_names[index % len(asha_names)], ago(hours=created_hours)),
#         )
...
#                 prescription = prescriptions.get(index, "Review after 3 days.")
...
#                  doctor_notes, created_at, acknowledged_at, acknowledged_by, completed_at)
#                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
#                 """,
#                 (referral_id, abha_map[index], triage_id, facility, status,
#                  prescription, notes, ago(hours=created_hours),
...
#         VALUES (?, ?, ?, ?, ?, ?, ?, ?)
#         """,
#         (str(uuid.uuid4()), abha_map[3], "Imran Sheik", "Shirval",
#          "ASHA Kavita (RED triage)", "ACTIVE", ago(hours=1), None),
#     )

#     cur.execute(
#         "INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#         (str(uuid.uuid4()), abha_map[1], "District Hospital Pune",
#          (date.today() + timedelta(days=1)).isoformat(), "10:00-11:00",
#          "Fever follow-up", "REQUESTED", ago(hours=2)),
#     )

#     cur.execute(
#         "INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#         (str(uuid.uuid4()), abha_map[0], "PHC Ranjangaon",
#          (date.today() + timedelta(days=2)).isoformat(), "11:00-12:00",
#          "Ante-natal check-up", "CONFIRMED", ago(hours=8)),
#     )

#     diagnostic_examples = [
#         (abha_map[1], "District Hospital Pune", "CBC", "COMPLETED"),
#         (abha_map[4], "Sub-Centre Wagholi", "Hb", "ORDERED"),
#     ]

#     for patient_id, facility, test_name, status in diagnostic_examples:
#         referral = cur.execute(
#             "SELECT id FROM referrals WHERE abha_id = ? ORDER BY created_at DESC LIMIT 1",
#             (patient_id,),
#         ).fetchone()
...
#             (str(uuid.uuid4()), patient_id, referral_id, facility, test_name,
#              status, ago(hours=3), "Normal" if status == "COMPLETED" else None),
#         )

#     notifications = [
#         ("admin", "🚨 Active SOS: Imran Sheik from Shirval."),
#         ("doctor", "📅 New appointment request received."),
#         ("asha", "💊 Referral completed and prescription issued."),
#         ("admin", "📊 Diagnostics load updated."),
#     ]

#     for role, message in notifications:
#         cur.execute(
#             "INSERT INTO notifications VALUES (?, ?, ?, 0, ?)",
#             (str(uuid.uuid4()), role, message, ago(hours=random.randint(1, 5))),
#         )

#     # -------------------------------------------------------------------------
#     # NEW (v4): seed a declining consumption trend for 2 medicines so the
#     # predictive stockout forecaster has something meaningful to show
#     # immediately in the demo, instead of an empty "not enough data" state.
#     # -------------------------------------------------------------------------
#     trend_targets = cur.execute(
#         "SELECT id, facility, medicine_name, quantity FROM medicine_stock "
#         "WHERE medicine_name IN ('Paracetamol 500mg', 'ORS Sachets') LIMIT 2"
#     ).fetchall()

#     for stock_id, facility, medicine_name, current_qty in trend_targets:
#         running_qty = current_qty + 25  # pretend stock started higher 10 days ago
#         history_entries = []
#         for day_offset in range(10, 0, -2):
#             drop = random.randint(3, 6)
#             running_qty = max(0, running_qty - drop)
#             history_entries.append((running_qty, -drop, ago(days=day_offset)))

#         final_qty = history_entries[-1][0]
#         cur.execute(
#             "UPDATE medicine_stock SET quantity = ?, last_updated = ? WHERE id = ?",
#             (final_qty, ago(days=0), stock_id),
#         )
#         for qty_after, change_amt, ts in history_entries:
#             cur.execute(
#                 """
#                 INSERT INTO medicine_stock_history
#                 (id, stock_id, facility, medicine_name, quantity_after, change_amount, changed_at)
#                 VALUES (?,?,?,?,?,?,?)
#                 """,
#                 (str(uuid.uuid4()), stock_id, facility, medicine_name, qty_after, change_amt, ts),
#             )

#     # -------------------------------------------------------------------------
#     # NEW (v4): seed a couple of access-log entries for the audit trail demo
#     # -------------------------------------------------------------------------
#     cur.execute(
#         "INSERT INTO access_log VALUES (?,?,?,?,?)",
#         (str(uuid.uuid4()), abha_map[1], "Dr. Anil (DH)",
#          "Completed referral & issued prescription", ago(hours=22)),
#     )
#     cur.execute(
#         "INSERT INTO access_log VALUES (?,?,?,?,?)",
#         (str(uuid.uuid4()), abha_map[0], "Patient (self)",
#          "Downloaded ABDM-ready health record (JSON export)", ago(hours=5)),
#     )
...
#         {"type": record_type, "values": values}
#     )
...
#         abha_id = values[0]
#         name = values[1]
#         options[f"{name} ({abha_id}) [offline]"] = abha_id
#     return options
...
#             cur.execute("INSERT OR IGNORE INTO triage VALUES (?, ?, ?, ?, ?, ?, ?)", values)
#             synced += 1
...
#         type="primary",
#     ):
#         if secrets.compare_digest(pin, DEMO_PINS[role_key]):
...
#     patient_name, abha_id, facility, priority, symptoms_text,
#     prescription, doctor_notes, doctor_name, completed_at,
# ):
#     return "\n".join(
#         [
#             "=" * 50,
...
#     current_language_label = language_labels.get(st.session_state.ui_lang, "English")

#     selected_language = st.selectbox(
#         "Language / भाषा",
#         list(language_labels.values()),
#         index=list(language_labels.values()).index(current_language_label),
#     )

#     st.session_state.ui_lang = next(
#         code for code, label in language_labels.items() if label == selected_language
#     )
...
#     else:
#         page = st.radio(tr("role"), [r[0] for r in role_options])
#         st.session_state.selected_page = page

#     st.divider()

#     patient_count = int(df("SELECT COUNT(*) AS n FROM patients").iloc[0]["n"])
...
#     if GROQ_API_KEY:
#         st.caption("🤖 AI mode: Groq Llama 3.1 (live) + Whisper voice-to-text")
#     else:
#         st.caption("🤖 AI mode: Rule-based + vitals fallback (set GROQ_API_KEY for live AI)")
...
#         help="When enabled, ASHA records are stored in the session until Sync Now is pressed.",
#     )

#     queue_length = len(st.session_state.offline_queue)

#     if queue_length:
#         st.warning(f"📥 {tr('pending_sync')}: **{queue_length}**")
#         if st.button(tr("sync_now"), type="primary", use_container_width=True):
#             synced = sync_offline_queue()
#             st.success(tr("synced_msg").format(n=synced))
#             st.rerun()
#     elif st.session_state.offline_mode:
#         st.caption("No records queued yet.")
...
#         notifications = df(
#             "SELECT message, created_at FROM notifications WHERE role = ? "
...
#         st.divider()
#         st.error(f"🚨 {active_alert_count} active emergency alert(s)")
...
#             if st.button(tr("login_btn"), type="primary", key="patient_login_button"):

#                 match = df(
#                     """
#                     SELECT abha_id, name, password_hash, salt FROM patients
#                     WHERE phone = ? AND password_hash IS NOT NULL LIMIT 1
#                     """,
#                     (login_phone,),
#                 )
...
#                 valid = (
#                     not match.empty
#                     and verify_password(login_password, match.iloc[0]["salt"], match.iloc[0]["password_hash"])
#                 )
...
#                 c1, c2 = st.columns(2)
#                 r_name = c1.text_input(tr("name"))
#                 r_age = c2.number_input(tr("age"), min_value=0, max_value=120, value=30)
#                 r_gender = c1.selectbox(tr("gender"), ["Female", "Male", "Other"])
#                 r_village = c2.text_input(tr("village"))
#                 r_phone = c1.text_input(tr("phone"))
#                 r_language = c2.selectbox(
#                     tr("language"), ["Marathi", "Hindi", "Tamil", "Telugu", "Bengali", "English"]
#                 )
#                 r_password = c1.text_input(tr("password"), type="password")
#                 r_confirm = c2.text_input(tr("confirm_password"), type="password")
...
#                     else:
#                         password_hash, salt = hash_password(r_password)
...
#                             """,
#                             (
#                                 new_abha_id, r_name.strip(), r_age, r_gender, r_village.strip(),
#                                 r_phone.strip(), r_language, datetime.now().isoformat(),
#                                 password_hash, salt, "Patient (self)",
#                             ),
#                         )
#                         conn.commit()
#                         conn.close()

#                         st.success(f"✅ Account created! Your ABHA-linked ID is **{new_abha_id}**.")
#                         maybe_render_qr(new_abha_id, "Your ABHA-linked ID")
...
#         col_a.subheader(f"👋 {tr('welcome_back')}, {patient['name']}")
#         if col_b.button(tr("logout")):
#             st.session_state.patient_logged_in = False
#             st.session_state.patient_abha_id = None
#             st.rerun()
...
#         c1.metric("ABHA ID", patient["abha_id"])
#         c2.metric(tr("age"), int(patient["age"]))
#         c3.metric(tr("village"), patient["village"] or "—")

#         maybe_render_qr(patient["abha_id"], "ABHA-linked ID")

#         # NEW (v4): every export is now logged to the audit trail
#         abdm_export_data = build_abdm_export(patient["abha_id"])
#         if st.download_button(
#             "⬇️ Download ABDM-Ready Health Record (JSON)",
#             data=abdm_export_data,
#             file_name=f"abdm_record_{patient['abha_id']}.json",
#             mime="application/json",
#             key="patient_abdm_download",
#         ):
#             log_access(patient["abha_id"], "Patient (self)", "Downloaded ABDM-ready health record (JSON export)")

#         b1, b2 = st.columns(2)

#         with b1:
#             with st.expander("📅 " + tr("book_appointment")):
#                 with st.form("appointment_form"):
#                     facility = st.selectbox("Facility", FACILITIES)
#                     appointment_date = st.date_input("Preferred date", min_value=date.today())
#                     slot = st.selectbox("Slot", APPOINTMENT_SLOTS)
...
#                             "INSERT INTO appointments VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#                             (
#                                 str(uuid.uuid4()), patient["abha_id"], facility,
#                                 appointment_date.isoformat(), slot, reason.strip(),
...
#                         notify(
#                             "doctor",
#                             f"📅 New appointment request from {patient['name']} at "
#                             f"{facility} on {appointment_date} ({slot})",
#                         )
#                         st.success("Appointment requested.")

#         with b2:
#             st.markdown("### 🚨 Emergency")
#             if st.button("🚨 " + tr("sos"), type="primary", use_container_width=True):
#                 conn = get_conn()
#                 conn.execute(
#                     "INSERT INTO emergency_alerts VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#                     (
#                         str(uuid.uuid4()), patient["abha_id"], patient["name"], patient["village"],
#                         "Patient Portal (self-reported)", "ACTIVE", datetime.now().isoformat(), None,
#                     ),
#                 )
#                 conn.commit()
#                 conn.close()

#                 notify("admin", f"🚨 Self-reported SOS: {patient['name']} ({patient['village']})")
#                 st.error(tr("sos_logged"))

#         appointments = df(
#             "SELECT appointment_date, slot, reason, status FROM appointments "
#             "WHERE abha_id = ? ORDER BY appointment_date DESC",
#             (abha_id,),
#         )
...
#             ["🗺 Full Journey", "🩺 " + tr("my_triage_history"),
#              "📨 " + tr("my_referrals"), "🧪 Diagnostics"]
#         )

#         with journey_tab:
#             events = get_patient_timeline(abha_id)
#             if not events:
...
#                     st.markdown(
#                         f"""
#                         <div style="border-left:4px solid {event['color']};padding-left:12px;margin:12px 0;">
#                             <strong>{event['kind']}</strong> — {event['detail']}<br>
...
#             triage_history = df(
#                 "SELECT symptoms_text, priority, rationale, created_by, created_at "
...
#                 st.info(tr("no_records_yet"))
#             else:
#                 st.dataframe(triage_history, use_container_width=True, hide_index=True)

#                 # NEW (v4): offline text-to-speech readout of the latest result
#                 latest = triage_history.iloc[0]
#                 lang = st.session_state.ui_lang
#                 phrase = PRIORITY_SPEECH.get(lang, PRIORITY_SPEECH["en"]).get(latest["priority"], "")
#                 if phrase:
#                     st.caption("Your most recent result, read aloud:")
#                     speak_button(phrase, TTS_LANG_CODES.get(lang, "en-IN"), key=f"speak_patient_{abha_id}")

#         with referral_tab:
#             referral_history = df(
#                 """
#                 SELECT r.facility, r.status, r.prescription, r.doctor_notes, r.created_at,
#                        r.acknowledged_at, r.acknowledged_by, r.completed_at, t.priority, t.symptoms_text
#                 FROM referrals r JOIN triage t ON t.id = r.triage_id
#                 WHERE r.abha_id = ? ORDER BY r.created_at DESC
#                 """,
#                 (abha_id,),
#             )
...
#                 st.dataframe(
#                     referral_history[
#                         ["facility", "status", "prescription", "doctor_notes",
...
#                 completed_rows = referral_history[referral_history["status"] == "COMPLETED"]

#                 if not completed_rows.empty:
#                     st.caption("📄 Download completed prescriptions:")

#                     for i, row in completed_rows.iterrows():
#                         prescription_text = build_prescription_text(
#                             patient["name"], patient["abha_id"], row["facility"], row["priority"],
#                             row["symptoms_text"], decrypt_field(row["prescription"]),
#                             decrypt_field(row["doctor_notes"]), row["acknowledged_by"], row["completed_at"],
#                         )

#                         # NEW (v4): download events feed the audit trail
#                         if st.download_button(
#                             f"⬇️ {row['facility']} — {row['completed_at']}",
#                             data=prescription_text,
#                             file_name=f"prescription_{abha_id}_{i}.txt",
#                             key=f"patient_prescription_{i}",
#                         ):
#                             log_access(abha_id, "Patient (self)", f"Downloaded prescription from {row['facility']}")

#         with diagnostic_tab:
#             diagnostics = df(
#                 "SELECT test_name, facility, status, ordered_at, result_notes "
#                 "FROM diagnostics WHERE abha_id = ? ORDER BY ordered_at DESC",
#                 (abha_id,),
#             )
#             if diagnostics.empty:
#                 st.info("No diagnostics ordered yet.")
#             else:
...
#     if not require_pin("asha", tr("asha")):
#         st.stop()
...
#         st.info(
#             "📡 Offline mode is ON. New ASHA records will remain locally queued until synchronization."
#         )

#     register_tab, triage_tab = st.tabs(
#         ["📝 " + tr("register_patient"), "🤖 " + tr("run_triage") + " & Referral"]
#     )
...
#                 tr("language"), ["Marathi", "Hindi", "Tamil", "Telugu", "Bengali", "English"]
#             )
#             worker_name = c1.text_input("Your name (ASHA/ANM)", value="ASHA Worker")

#             setup_login = st.checkbox(tr("create_login"))
#             asha_password = ""
#             if setup_login:
#                 asha_password = st.text_input(tr("password"), type="password")
...
#                     st.error("Enter a password or disable app login.")
#                 else:
#                     abha_id = gen_abha_id()

#                     if setup_login:
#                         password_hash, salt = hash_password(asha_password)
#                     else:
#                         password_hash, salt = None, None

#                     values = (
#                         abha_id, name.strip(), age, gender, village.strip(), phone.strip(),
#                         patient_language, datetime.now().isoformat(), password_hash, salt,
#                         worker_name.strip(),
#                     )

#                     if st.session_state.offline_mode:
#                         queue_offline_record("patient", values)
#                         st.info(f"💾 {tr('saved_offline')} ABHA ID: **{abha_id}**")
#                     else:
#                         conn = get_conn()
#                         conn.execute(
#                             """
...
#                         st.success(f"✅ Patient registered! ABHA ID: **{abha_id}**")
#                         maybe_render_qr(abha_id, "Patient ABHA-linked ID")

#                     if setup_login:
#                         st.info(
#                             "Patient can now use the Patient Portal with their registered phone and password."
#                         )
...
#         patients = df(
#             "SELECT abha_id, name, age, gender, village, phone, language, created_by, created_at "
...
#         patients = df("SELECT abha_id, name FROM patients ORDER BY created_at DESC")

#         options = {f"{row['name']} ({row['abha_id']})": row["abha_id"] for _, row in patients.iterrows()}
#         options.update(get_offline_patient_options())
...
#             with st.expander("🗺 Recent patient journey"):
#                 events = get_patient_timeline(abha_id)[:5]
#                 if not events:
#                     st.caption("No synced history yet.")
#                 else:
#                     for event in events:
...
#             # -----------------------------------------------------------------
#             # NEW (v4): optional voice-to-text symptom capture (Groq Whisper)
#             # -----------------------------------------------------------------
#             st.markdown("#### " + tr("voice_input_label"))
#             audio_value = None
#             try:
#                 audio_value = st.audio_input(
#                     "Record symptoms in your language", key="asha_voice_input"
#                 )
#             except AttributeError:
#                 st.caption(
#                     "Voice recording needs a newer Streamlit version — "
#                     "please type the symptoms below instead."
#                 )

#             voice_transcript = ""
#             if audio_value is not None:
#                 if GROQ_API_KEY:
#                     with st.spinner(tr("voice_transcribing")):
#                         transcript = transcribe_voice_note(audio_value.getvalue())
#                     if transcript:
#                         voice_transcript = transcript
#                         st.success(f"🎙️ Transcribed: {voice_transcript}")
#                     else:
#                         st.warning("Voice transcription failed — please type the symptoms instead.")
#                 else:
#                     st.info(
#                         "Set GROQ_API_KEY to enable automatic voice-to-text transcription. "
#                         "The recording is captured but must be typed manually for now."
#                     )
...
#                 placeholder=(
#                     "Example: High fever for 3 days with breathlessness / "
#                     "बुखार और सांस लेने में तकलीफ"
#                 ),
#                 height=120,
#             )

#             with st.expander("🩺 " + tr("vitals_section")):
#                 st.caption(tr("vitals_hint"))
#                 v1, v2, v3, v4, v5 = st.columns(5)
#                 temp_f = v1.number_input(tr("temp_label"), min_value=0.0, max_value=112.0, value=0.0, step=0.1)
#                 spo2 = v2.number_input(tr("spo2_label"), min_value=0, max_value=100, value=0, step=1)
#                 pulse = v3.number_input(tr("pulse_label"), min_value=0, max_value=220, value=0, step=1)
#                 bp_sys = v4.number_input(tr("bp_sys_label"), min_value=0, max_value=260, value=0, step=1)
#                 bp_dia = v5.number_input(tr("bp_dia_label"), min_value=0, max_value=200, value=0, step=1)

#             worker_name = st.text_input("ASHA/ANM name", value="ASHA Worker", key="triage_worker_name")
...
#                 else:
#                     vitals = {"temp_f": temp_f, "spo2": spo2, "pulse": pulse, "bp_sys": bp_sys, "bp_dia": bp_dia}

#                     with st.spinner("Running triage..."):
#                         priority, rationale = run_triage(symptoms, vitals)

#                     triage_id = str(uuid.uuid4())
#                     triage_values = (
#                         triage_id, abha_id, symptoms.strip(), priority, rationale,
#                         worker_name.strip(), datetime.now().isoformat(),
#                     )

#                     if st.session_state.offline_mode:
#                         queue_offline_record("triage", triage_values)
#                         st.info(f"💾 {tr('saved_offline')}")
#                     else:
#                         conn = get_conn()
#                         conn.execute("INSERT INTO triage VALUES (?, ?, ?, ?, ?, ?, ?)", triage_values)
#                         conn.commit()
#                         conn.close()
...
#                     # NEW (v4): offline, zero-cost read-aloud of the result
#                     lang = st.session_state.ui_lang
#                     phrase = PRIORITY_SPEECH.get(lang, PRIORITY_SPEECH["en"]).get(priority, "")
#                     if phrase:
#                         speak_button(phrase, TTS_LANG_CODES.get(lang, "en-IN"), key=f"speak_asha_{triage_id}")
...
#                 triage_abha_id = st.session_state.get("last_triage_abha_id", abha_id)

#                 if priority in ("RED", "YELLOW"):

#                     st.warning("This patient needs facility follow-up.")

#                     if priority == "RED":
#                         if st.button(tr("trigger_sos"), type="primary"):
#                             patient_lookup = df(
#                                 "SELECT name, village FROM patients WHERE abha_id = ?", (triage_abha_id,)
#                             )
#                             if not patient_lookup.empty:
#                                 patient_name = patient_lookup.iloc[0]["name"]
#                                 patient_village = patient_lookup.iloc[0]["village"]
#                             else:
#                                 patient_name = selected.split(" (")[0]
#                                 patient_village = "Unknown"

#                             conn = get_conn()
#                             conn.execute(
#                                 "INSERT INTO emergency_alerts VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#                                 (
#                                     str(uuid.uuid4()), triage_abha_id, patient_name, patient_village,
#                                     f"ASHA: {worker_name}", "ACTIVE", datetime.now().isoformat(), None,
#                                 ),
#                             )
#                             conn.commit()
#                             conn.close()
...
#                             notify(
#                                 "admin",
#                                 f"🚨 SOS: {patient_name} ({patient_village}) — escalated by {worker_name}",
#                             )
#                             st.error(tr("sos_logged"))

#                     facility = st.selectbox(tr("select_facility"), FACILITIES, key="asha_referral_facility")

#                     # NEW (v4): facility load-balancing hint at the point of referral
#                     loads = facility_pending_load()
#                     st.caption(
#                         "📊 " + tr("facility_load_hint") + ": "
#                         + ", ".join(f"{f} ({loads.get(f, 0)})" for f in FACILITIES)
#                     )
#                     least_loaded = min(loads, key=loads.get)
#                     if loads.get(facility, 0) - loads.get(least_loaded, 0) >= 3 and least_loaded != facility:
#                         st.warning(
#                             f"⚠️ {facility} currently has {loads.get(facility, 0)} pending referrals "
#                             f"awaiting acknowledgment. {least_loaded} has fewer ({loads.get(least_loaded, 0)}) "
#                             "— consider it if clinically appropriate."
#                         )
...
#                         referral_id = str(uuid.uuid4())
#                         referral_values = (
#                             referral_id, triage_abha_id, last_triage_id, facility, "PENDING",
#                             "", "", datetime.now().isoformat(), None, None, None,
#                         )

#                         if st.session_state.offline_mode:
#                             queue_offline_record("referral", referral_values)
#                             st.info(f"💾 {tr('saved_offline')} Referral queued for {facility}.")
#                         else:
#                             conn = get_conn()
#                             conn.execute(
...
#                             notify("doctor", f"📨 New referral for {facility}.")
#                             st.success(f"Referral sent to {facility}. Status: PENDING.")

#                         st.session_state.pop("last_triage_id", None)

#                 else:
#                     st.info("GREEN priority — routine care. No referral required.")
...
#     if not require_pin("doctor", tr("doctor")):
#         st.stop()

#     facility = st.selectbox("Facility", FACILITIES)
#     doctor_name = st.text_input(tr("doctor_name_label"), value="Duty Doctor")
#     name_filter = st.text_input("🔎 " + tr("search_patient"))

#     queue = df(
#         """
#         SELECT r.id AS ref_id, p.abha_id, p.name, p.age, p.gender, p.village,
#                t.symptoms_text, t.priority, t.rationale, t.created_by,
#                r.status, r.created_at, r.acknowledged_at, r.acknowledged_by
#         FROM referrals r
...
#     if name_filter.strip() and not queue.empty:
#         queue = queue[queue["name"].str.contains(name_filter, case=False, na=False)]
...
#     else:
#         queue["priority_sort"] = queue["priority"].map(PRIORITY_ORDER)
#         queue["status_sort"] = queue["status"].map(STATUS_ORDER)
#         queue = queue.sort_values(["status_sort", "priority_sort"])

#         for _, row in queue.iterrows():

#             with st.expander(
#                 f"{row['name']} · {row['age']}y · {row['priority']} · {row['status']}",
#                 expanded=(row["status"] == "PENDING"),
#             ):

#                 st.markdown(
#                     f"{priority_badge(row['priority'])} &nbsp; {status_badge(row['status'])}",
#                     unsafe_allow_html=True,
#                 )
...
#                 st.caption(f"Referred on: {row['created_at']}")

#                 if row["status"] == "PENDING":

#                     st.warning("🚦 " + tr("awaiting_arrival"))

#                     # NEW (v4): SLA / accountability timer
#                     elapsed = hours_since(row["created_at"])
#                     sla_limit = SLA_HOURS.get(row["priority"])
#                     if elapsed is not None and sla_limit:
#                         if elapsed > sla_limit:
#                             st.error(f"⏰ {tr('sla_breach')} — referred {elapsed:.1f}h ago (SLA: {sla_limit}h)")
#                         else:
#                             st.caption(f"⏱️ {tr('sla_ok')}: {sla_limit - elapsed:.1f}h remaining")

#                     if st.button(tr("acknowledge_btn"), key=f"ack_{row['ref_id']}", type="primary"):
...
#                             WHERE id = ?
#                             """,
#                             (now, doctor_name, row["ref_id"]),
#                         )
#                         conn.commit()
#                         conn.close()

#                         notify("asha", f"✅ {row['name']} arrived at {facility}.")
#                         notify("admin", f"✅ Arrival acknowledged: {row['name']} at {facility}.")
#                         log_access(row["abha_id"], doctor_name, f"Acknowledged referral arrival at {facility}")

#                         st.success("Patient arrival acknowledged.")
#                         st.rerun()

#                 elif row["status"] == "ACKNOWLEDGED":

#                     st.success(
#                         f"✅ {tr('acknowledged_on')} {row['acknowledged_at']} "
#                         f"({tr('acknowledged_by')}: {row['acknowledged_by']})"
#                     )

#                     tc1, tc2 = st.columns(2)
#                     with tc1:
#                         if st.button("📹 " + tr("teleconsult"), key=f"tc_{row['ref_id']}"):
#                             st.info("Teleconsultation room placeholder. Integrate Daily.co/WebRTC here.")
#                     with tc2:
#                         st.write(f"Referral status: `{row['status']}`")

#                     st.markdown("### 🧪 Diagnostics")
#                     tests = st.multiselect("Order diagnostics", DIAGNOSTIC_TESTS, key=f"diag_{row['ref_id']}")

#                     if st.button("🏥 Order Selected Tests", key=f"order_diag_{row['ref_id']}"):
#                         if not tests:
...
#                                     "INSERT INTO diagnostics VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
#                                     (
#                                         str(uuid.uuid4()), row["abha_id"], row["ref_id"], facility,
#                                         test, "ORDERED", datetime.now().isoformat(), None,
#                                     ),
#                                 )
#                             conn.commit()
#                             conn.close()

#                             notify("asha", f"🧪 {len(tests)} diagnostic test(s) ordered for {row['name']}.")
#                             log_access(row["abha_id"], doctor_name, f"Ordered {len(tests)} diagnostic test(s)")
#                             st.success(f"{len(tests)} test(s) ordered.")

#                     prescription = st.text_area(tr("prescription"), key=f"prescription_{row['ref_id']}", height=150)
#                     notes = st.text_area("Doctor notes", key=f"notes_{row['ref_id']}")

#                     if st.button(tr("save_prescription"), key=f"complete_{row['ref_id']}", type="primary"):
...
#                             """
#                             UPDATE referrals
#                             SET status = 'COMPLETED', prescription = ?, doctor_notes = ?, completed_at = ?
#                             WHERE id = ?
#                             """,
#                             (encrypt_field(prescription), encrypt_field(notes), completed_at, row["ref_id"]),
#                         )
#                         conn.commit()
#                         conn.close()

#                         notify("asha", f"💊 Prescription issued for {row['name']} — referral completed.")
#                         notify("admin", f"💊 Referral completed: {row['name']} at {facility}.")
#                         log_access(row["abha_id"], doctor_name, "Completed referral & issued prescription")

#                         st.success("Referral completed and prescription saved.")

#                         prescription_text = build_prescription_text(
#                             row["name"], row["abha_id"], facility, row["priority"], row["symptoms_text"],
#                             prescription, notes, doctor_name, completed_at,
#                         )

#                         st.download_button(
#                             tr("download_prescription"),
#                             data=prescription_text,
#                             file_name=f"prescription_{row['abha_id']}.txt",
...
#     history = df(
#         """
#         SELECT p.name, t.priority, r.acknowledged_at, r.prescription, r.completed_at
...
#     st.divider()
#     st.subheader("📅 Facility Appointments")

#     appointments = df(
#         """
#         SELECT a.id, a.abha_id, p.name, a.appointment_date, a.slot, a.reason, a.status
#         FROM appointments a JOIN patients p ON p.abha_id = a.abha_id
#         WHERE a.facility = ? ORDER BY a.appointment_date, a.slot
#         """,
#         (facility,),
#     )

#     if appointments.empty:
#         st.info("No appointments for this facility.")
#     else:
#         for _, appointment in appointments.iterrows():
#             c1, c2, c3 = st.columns([5, 1, 1])
#             c1.write(
#                 f"**{appointment['name']}** — {appointment['appointment_date']} "
#                 f"{appointment['slot']} — {appointment['reason']} ({appointment['status']})"
#             )

#             if appointment["status"] == "REQUESTED":
#                 if c2.button("Confirm", key=f"confirm_{appointment['id']}"):
#                     conn = get_conn()
#                     conn.execute("UPDATE appointments SET status = 'CONFIRMED' WHERE id = ?", (appointment["id"],))
#                     conn.commit()
#                     conn.close()
#                     notify("asha", f"📅 Appointment confirmed for {appointment['name']} at {facility}.")
#                     st.rerun()

#                 if c3.button("Decline", key=f"decline_{appointment['id']}"):
#                     conn = get_conn()
#                     conn.execute("UPDATE appointments SET status = 'DECLINED' WHERE id = ?", (appointment["id"],))
#                     conn.commit()
#                     conn.close()
#                     st.rerun()

#             elif appointment["status"] == "CONFIRMED":
#                 if c2.button("Mark Done", key=f"done_{appointment['id']}"):
#                     conn = get_conn()
#                     conn.execute("UPDATE appointments SET status = 'DONE' WHERE id = ?", (appointment["id"],))
#                     conn.commit()
#                     conn.close()
#                     st.rerun()
#             else:
#                 c2.write(appointment["status"])
...
#         # NEW (v4): SLA breach accountability metric
#         pending_with_priority = df(
#             """
#             SELECT r.created_at, t.priority
#             FROM referrals r JOIN triage t ON t.id = r.triage_id
#             WHERE r.status = 'PENDING'
#             """
#         )
#         breach_count = 0
#         if not pending_with_priority.empty:
#             for _, prow in pending_with_priority.iterrows():
#                 sla_limit = SLA_HOURS.get(prow["priority"])
#                 elapsed = hours_since(prow["created_at"])
#                 if sla_limit and elapsed is not None and elapsed > sla_limit:
#                     breach_count += 1
#         st.metric("⏰ Open SLA Breaches (accountability)", breach_count)
...
#             # NEW (v4): facility health score — blends referral performance
#             # with medicine stock health into one prioritization number
#             stock_health = df(
#                 """
#                 SELECT facility,
#                        ROUND(100.0 * SUM(CASE WHEN quantity >= 10 THEN 1 ELSE 0 END) / COUNT(*), 1) AS stock_health_pct
#                 FROM medicine_stock GROUP BY facility
#                 """
#             )
#             facility_data = facility_data.merge(stock_health, on="facility", how="left").fillna(0)
#             facility_data["health_score"] = (
#                 facility_data["completion_rate"] * 0.4
#                 + facility_data["arrival_rate"] * 0.3
#                 + facility_data["stock_health_pct"] * 0.3
#             ).round(1)
...
#                     "stock_health_pct": "Stock health %", "health_score": tr("facility_health_score"),
#                 }
#             )
...
#             st.caption(
#                 "🏆 " + tr("facility_health_score")
#                 + " = a weighted blend of referral completion, arrival rate & medicine stock "
#                 "health — one number to prioritize which facility needs support first."
#             )
#             st.bar_chart(facility_data.set_index("facility")[["health_score"]])
#         else:
...
#         # NEW (v4): predictive stockout forecasting, beyond a static threshold
#         st.divider()
#         st.subheader("📉 Predictive Stockout Forecast")
#         st.caption(
#             "Unlike a static low-stock threshold, this uses actual consumption "
#             "velocity from restock/dispense history to estimate days remaining."
#         )
#         all_stock_items = df("SELECT id, facility, medicine_name, quantity, unit FROM medicine_stock")
#         forecast_rows = []
#         for _, item in all_stock_items.iterrows():
#             days_left = predict_stockout(item["id"])
...
#                     "Unit": item["unit"],
#                     "Predicted stockout (days)": days_left,
#                 })
#         if forecast_rows:
#             forecast_df = pd.DataFrame(forecast_rows).sort_values("Predicted stockout (days)")
#             st.dataframe(forecast_df, use_container_width=True, hide_index=True)
#         else:
#             st.success("No medicines are predicted to stock out within 7 days (or not enough history yet).")
...
#             "- ✅ Consent & access audit trail for every referral action and record export (see below)\n"
...
#         # NEW (v4): consent & access audit trail viewer
#         st.divider()
#         st.subheader("🔍 " + tr("audit_trail"))
#         st.caption(
#             "Every referral acknowledgment, prescription, and patient record export is logged "
#             "here — supporting ABDM-style consent accountability, not just data storage."
#         )
#         audit_trail = df(
#             "SELECT abha_id, accessed_by, purpose, created_at FROM access_log "
#             "ORDER BY created_at DESC LIMIT 30"
#         )
#         if audit_trail.empty:
#             st.caption("No access events logged yet.")
#         else:
...
#     stock = df(query, tuple(params))

#     if not stock.empty:
#         # NEW (v4): predictive stockout column, computed from consumption history
#         stock["Predicted stockout (days)"] = stock["id"].apply(predict_stockout)

#     if stock.empty:
#         st.info("No matching medicine records.")
#     else:
#         display_cols = [
#             "facility", "medicine_name", "quantity", "unit",
#             "last_updated", "Predicted stockout (days)",
#         ]
#         display_stock = stock[display_cols]

#         def highlight_low(row):
#             if row["quantity"] < 10:
#                 return ["background-color:#ffccd2;color:#000" for _ in row]
#             return ["" for _ in row]

#         styled_stock = display_stock.style.apply(highlight_low, axis=1)

#         st.dataframe(styled_stock, use_container_width=True, hide_index=True)
#         st.caption(
#             "🔴 Highlighted rows indicate quantity below 10. Stockout forecast uses "
#             "consumption history where available — blank means not enough history yet."
#         )

#         st.download_button(
#             tr("download_csv"),
#             data=display_stock.to_csv(index=False),
...
#         all_medicines = df(
#             "SELECT id, facility, medicine_name, quantity, unit FROM medicine_stock "
...
#             labels = {
#                 (
#                     f"{row['medicine_name']} — {row['facility']} "
#                     f"(currently: {row['quantity']} {row['unit']})"
#                 ): row["id"]
#                 for _, row in all_medicines.iterrows()
#             }

#             selected_label = st.selectbox("Select medicine", list(labels.keys()))
#             selected_id = labels[selected_label]

#             action = st.radio(
#                 "Action",
#                 ["Add stock (restock delivery)", "Remove stock (dispensed/damaged)", "Set exact quantity"],
#             )
...
#                 result = conn.execute(
#                     "SELECT quantity, facility, medicine_name FROM medicine_stock WHERE id = ?",
#                     (selected_id,),
#                 ).fetchone()
...
#                 # NEW (v4): log the delta so the predictive forecaster has
#                 # real consumption/restock velocity to learn from
#                 change_amount = new_quantity - current_quantity
#                 now_iso = datetime.now().isoformat()

#                 conn.execute(
#                     "UPDATE medicine_stock SET quantity = ?, last_updated = ? WHERE id = ?",
#                     (new_quantity, now_iso, selected_id),
#                 )
#                 conn.execute(
#                     """
#                     INSERT INTO medicine_stock_history
...
#                     VALUES (?,?,?,?,?,?,?)
#                     """,
#                     (str(uuid.uuid4()), selected_id, facility_name, medicine_name,
#                      new_quantity, change_amount, now_iso),
#                 )

#                 conn.commit()
#                 conn.close()

#                 st.success(f"Inventory updated. New quantity: {new_quantity}")
#                 st.rerun()
...
#             new_facility = st.selectbox("Facility", FACILITIES, key="new_medicine_facility")
#             new_name = st.text_input("Medicine name")
#             new_quantity = st.number_input("Starting quantity", min_value=0, value=50, step=1)
...
#                         (new_stock_id, new_facility, new_name.strip(), new_quantity, new_unit, now_iso),
#                     )
#                     conn.execute(
#                         """
#                         INSERT INTO medicine_stock_history
...
#                         VALUES (?,?,?,?,?,?,?)
#                         """,
#                         (str(uuid.uuid4()), new_stock_id, new_facility, new_name.strip(),
#                          new_quantity, new_quantity, now_iso),
#                     )
#                     conn.commit()
#                     conn.close()

#                     st.success(f"Added {new_name} ({new_quantity} {new_unit}) to {new_facility}.")
#                     st.rerun()