...
#             c1.error(
#                 f"🚨 **{alert['patient_name']}** "
#                 f"({alert['village']}) — "
#                 f"reported by "
#                 f"*{alert['triggered_by']}* "
#                 f"at {alert['created_at']}"
#             )
...
#     st.divider()
#     st.subheader("🇮🇳 Interoperability & Compliance Checklist")
#     st.markdown(
#         "- ✅ **ABDM-ready ID** — every patient issued a unique ABHA-linked ID at registration\n"
#         "- ✅ **Structured export** — patient records downloadable as FHIR-lite JSON\n"
#         "- ✅ **Encryption at rest** — prescriptions & doctor notes stored encrypted "
#         f"({'active — cryptography installed' if _get_cipher() else 'inactive — run `pip install cryptography`'})\n"
#         "- ✅ **Offline-first sync** — timestamp-based conflict resolution on reconnect\n"
#         "- 🔜 **Full ABDM API integration** — planned for production rollout"
#     )
#     # -------------------------------------------------------------------------
#     # Social / Economic / Environmental Impact Estimator
#     # (directly answers the "Impact & Benefits" slide of the pitch deck)
#     # -------------------------------------------------------------------------

#     st.divider()

#     st.subheader(
#         "🌍 Social, Economic & Environmental Impact (Estimated)"
#     )
...
#     avg_km_per_trip = imp1.slider(
#         "Avg. one-way distance avoided per case (km)",
#         min_value=1, max_value=60, value=15,
#     )

#     cost_per_km = imp2.slider(
#         "Assumed cost per km (₹)",
#         min_value=1, max_value=30, value=8,
#     )

#     co2_per_km = imp3.slider(
#         "CO₂ emitted per km avoided (kg)",
...
#     # Every GREEN triage = a trip avoided entirely (ASHA handled it locally).
#     # Every COMPLETED referral = the visit was closed-loop instead of an
#     # open-ended repeat trip — used here as a conservative efficiency proxy.
#     cases_with_avoided_travel = green_triage_count + completed_referrals
#     total_km_saved = cases_with_avoided_travel * avg_km_per_trip * 2  # round trip
...
#     st.caption(
#         "Social impact: routine (GREEN) cases are resolved by the ASHA worker "
#         "without any facility visit, and every completed referral reflects a "
#         "patient who did not fall through the referral gap — both reduce "
#         "unnecessary travel and waiting time for rural patients."
#     )
...
#             SUM(
#                 CASE
#                     WHEN status IN ('ACKNOWLEDGED', 'COMPLETED')
#                     THEN 1
#                     ELSE 0
#                 END
#             ) AS arrived,
...
#         facility_data["arrival_rate"] = (
#             facility_data["arrived"]
#             / facility_data["total"]
#             * 100
#         ).round(1)

#         facility_data["completion_rate"] = (
#             facility_data["completed"]
#             / facility_data["total"]
#             * 100
#         ).round(1)
...
#                 "arrived": "Arrived",
#                 "completed": "Completed",
#                 "avg_hrs_to_ack": "Avg hrs to acknowledge",
...
#     st.subheader(
#         "🏆 ASHA / ANM Performance Leaderboard"
#     )
...
#         worker_names.update(
#             registrations["created_by"].dropna()
#         )

#     if not triages.empty:
#         worker_names.update(
#             triages["created_by"].dropna()
#         )

#     if not referrals_by_asha.empty:
#         worker_names.update(
#             referrals_by_asha["created_by"].dropna()
#         )
...
#     st.bar_chart(
#         funnel.set_index("Status")
#     )

#     st.caption(
#         "Closed-loop referral tracking shows the patient journey "
#         "from referral creation through arrival acknowledgement "
#         "to completion."
#     )
...
#             "completed_at",
#         ]:
#             referral_times[column] = pd.to_datetime(
#                 referral_times[column],
#                 errors="coerce",
#             )

#         referral_times["hrs_to_ack"] = (
#             referral_times["acknowledged_at"]
#             - referral_times["created_at"]
#         ).dt.total_seconds() / 3600

#         referral_times["hrs_ack_to_completion"] = (
#             referral_times["completed_at"]
#             - referral_times["acknowledged_at"]
#         ).dt.total_seconds() / 3600
...
#         t1.metric(
#             "Avg. time to acknowledgement",
#             f"{referral_times['hrs_to_ack'].mean():.1f} hrs",
#         )

#         t2.metric(
#             "Avg. arrival → completion",
#             f"{referral_times['hrs_ack_to_completion'].mean():.1f} hrs",
#         )

#         by_facility = referral_times.groupby(
#             "facility"
#         )[
#             [
#                 "hrs_to_ack",
#                 "hrs_ack_to_completion",
#             ]
#         ].mean()
...
#         recent_triage["created_at"] = pd.to_datetime(
#             recent_triage["created_at"],
#             errors="coerce",
#         )
...
#             datetime.now()
#             - timedelta(
#                 days=OUTBREAK_LOOKBACK_DAYS
#             )
#         )

#         recent_triage = recent_triage[
#             recent_triage["created_at"] >= cutoff
#         ]

#         risky = recent_triage[
#             recent_triage["priority"].isin(
#                 ["RED", "YELLOW"]
#             )
#         ]

#         clusters = (
#             risky.groupby("village")
#             .size()
#             .reset_index(
#                 name="risk_case_count"
#             )
#             .sort_values(
#                 "risk_case_count",
#                 ascending=False,
...
#                 st.warning(
#                     f"⚠️ **{row['village']}**: "
#                     f"{row['risk_case_count']} "
#                     f"RED/YELLOW cases in the "
#                     f"last {OUTBREAK_LOOKBACK_DAYS} days."
#                 )
...
#         st.caption(
#             "Frequent symptom keywords from recent triages. "
#             "This is a screening signal, not a diagnosis."
#         )
...
#         FROM diagnostics d
#         JOIN patients p
#             ON p.abha_id = d.abha_id
#         ORDER BY d.ordered_at DESC
#         LIMIT 10
#         """
#     )
...
#     st.subheader(
#         "🩺 Recent Disease / Symptom Signals"
#     )
