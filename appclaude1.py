...
# Demo flow:
# ASHA registers patient (optionally OFFLINE) -> runs AI triage -> flags HIGH RISK
...
#   -> Admin Dashboard shows live analytics (referral funnel, triage risk, low stock)

# NEW in this version (merged from the Flask backend design):
#   1. Closed-loop referral tracking: referrals now move through
#      PENDING -> ACKNOWLEDGED (receiving facility confirms arrival) -> COMPLETED
#      instead of jumping straight from PENDING to COMPLETED. This is the
#      single most important USP for the pitch - it proves the patient
#      wasn't "lost" between facilities.
#   2. Offline-first demo mode: a sidebar toggle lets you simulate the
#      ASHA worker having no internet connectivity. While ON, new patient
#      registrations / triage runs / referrals are held in an in-session
#      "offline queue" instead of being written to the database. Turning
#      it OFF and clicking "Sync Now" pushes everything to the database -
#      this is the visual proof point for judges that the app works
#      without connectivity and later reconciles.
...
#         # --- new keys for closed-loop referral + offline mode ---
...
#         with ref_tab:
#             ref_hist = df(
#                 """SELECT facility, status, prescription, doctor_notes, created_at, acknowledged_at, completed_at
